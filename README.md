...
on-device, writing web code only. The answer so far is yes, with two caveats worth
...
model runs in the native build, behind a ~60-line Capacitor plugin over `CMMotionManager`.

`magnetometer.ts` tries both backends and reports which one answered, so this is checked
on each device rather than assumed.

That plugin is a sensor tap, not arithmetic: the model is still Stan and the sampling
still happens in wasm. `startMagnetometerUpdates` is the uncalibrated field —
`deviceMotion.magneticField` has already had the hard-iron offset removed, which is
the thing being estimated.

Both permissions are prompted, must be triggered from a tap, and need a secure context.
`npm run dev` serves over https for that reason.
...
Pick a signing team in Xcode and run on a device. `CMMotionManager` needs no
permission prompt — unlike `DeviceMotion` in the browser.
...
table with its compile and sample time.
...
assignment the stride model needs.

Moving from 0.1.2 to 0.3.0 changes no answer here: all five models return the same
posterior means to five decimal places under the same seed, which is what you would
expect from a release whose new language features none of these models use. It is
faster — 25 to 30% off the two cheap models, a few percent off the accelerometer —
and 124 KB larger: 526 KB to 650 KB, or 196 KB to 241 KB over the wire. The
published module still validates with relaxed SIMD disabled, so the iOS
instantiation failure has not come back.
